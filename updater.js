...
const GH_REPO = { owner: 'francoviiii', repo: 'fishermans-dashboard', branch: 'main', path: 'data.json' };
const GH_TOKEN_KEY = 'dash_gh_token';
...
async function ghGetSha(token){
  const {owner, repo, path, branch} = GH_REPO;
  const url = `https://api.github.com/repos/${owner}/${repo}/contents/${path}?ref=${branch}`;
  const r = await fetch(url, {headers: {Authorization: `Bearer ${token}`, Accept: 'application/vnd.github+json'}});
  if (r.status === 404) return null;
  if (r.status === 401) throw new Error('Chave de acesso inválida ou expirada.');
  if (!r.ok) throw new Error('Não consegui ler o arquivo no GitHub (HTTP ' + r.status + ').');
...
async function ghPublish(){
  ...
  const token = ghToken();
  if (!token){
    document.getElementById('pub-config').style.display = '';
    if (statusEl) statusEl.innerHTML = `<div class="up-msg">Cole sua chave de acesso do GitHub abaixo e clique em "Salvar chave".</div>`;
    return;
  }
  if (!state.archive || !state.archive.length){
    if (statusEl) statusEl.innerHTML = `<div class="up-msg err">⚠ Não há dados para publicar. Adicione ao menos um mês.</div>`;
    return;
  }
  try {
    if (statusEl) statusEl.innerHTML = `<div class="up-msg">Publicando para todos…</div>`;
    const {owner, repo, path, branch} = GH_REPO;
    const sha = await ghGetSha(token);
    const content = utf8ToBase64(JSON.stringify(serializeArchive(), null, 2));
    const body = {
      message: `Atualiza data.json · ${state.archive.length} mês(es) · ${new Date().toLocaleString('pt-BR')}`,
      content, branch,
    };
    if (sha) body.sha = sha;
    ...
    if (r.status === 401 || r.status === 403) throw new Error('Sem permissão. A chave precisa de "Contents: Read and write" neste repositório.');
    if (!r.ok){ const t = await r.text(); throw new Error('Falha ao publicar (HTTP ' + r.status + '). ' + t.slice(0,140)); }
    const months = state.archive.map(m => m.label).join(', ');
    if (statusEl) statusEl.innerHTML = `<div class="up-msg ok">✓ Publicado para todos! O site agora mostra: <strong>${months}</strong>.<br>Quem abrir o link verá os dados em até ~1 min (peça para recarregar com Ctrl+Shift+R se necessário).</div>`;
  } catch(e){
    if (statusEl) statusEl.innerHTML = `<div class="up-msg err">⚠ ${e.message}</div>`;
  }
}

function togglePubConfig(){
  const c = document.getElementById('pub-config');
  if (!c) return;
  c.style.display = c.style.display === 'none' ? '' : 'none';
  const tokenInput = document.getElementById('pub-token');
  if (tokenInput && ghToken()) tokenInput.placeholder = 'chave salva ✓ — cole uma nova para trocar';
}

function savePubToken(){
  const input = document.getElementById('pub-token');
  const statusEl = document.getElementById('pub-status');
  const val = (input?.value || '').trim();
  if (!val){ if (statusEl) statusEl.innerHTML = `<div class="up-msg err">⚠ Cole a chave antes de salvar.</div>`; return; }
  setGhToken(val);
  input.value = '';
  document.getElementById('pub-config').style.display = 'none';
  if (statusEl) statusEl.innerHTML = `<div class="up-msg ok">✓ Chave salva neste navegador. Agora clique em "Publicar para todos".</div>`;
}
...
function addMonth(){
  ...
  document.getElementById('updater-month').innerHTML = '';
  document.getElementById('updater-actions').innerHTML = `
    <button class="up-btn primary" id="up-publish">⇪ Publicar para todos</button>
    <button class="up-btn" id="up-download2">Baixar data.json</button>
    <button class="up-btn ghost" id="up-close">Fechar</button>`;
  document.getElementById('up-publish').addEventListener('click', ghPublish);
...
function initUpdater(){
  ...
  if (pubBtn) pubBtn.addEventListener('click', ghPublish);
  const pubCfgBtn = document.getElementById('pub-config-btn');
  if (pubCfgBtn) pubCfgBtn.addEventListener('click', togglePubConfig);
  const pubSave = document.getElementById('pub-save');
  if (pubSave) pubSave.addEventListener('click', savePubToken);
