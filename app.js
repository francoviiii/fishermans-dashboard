...
async function init(){
  try {
    // Override local (upload da planilha) tem prioridade
    const override = localStorage.getItem('dash_data_override');
    if (override) {
      state.data = JSON.parse(override);
    } else {
      // Tenta data.json na raiz (GitHub) e em dashboard/ (local)
